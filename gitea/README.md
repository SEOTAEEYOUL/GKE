...
### Install  
```
taeeyoul@cloudshell:~/workspace/ttc-infra/gitea/gitea$ helm install git . -n ttc-infra -f values.yaml
NAME: git
LAST DEPLOYED: Thu Aug 27 05:08:14 2020
NAMESPACE: ttc-infra
STATUS: deployed
REVISION: 1
TEST SUITE: None
...
1. Connect to your Gitea web URL by running:
  Ingress is enabled for this chart deployment.  Please access the web UI at git.team14.sk-ttc.com
2. Connect to your Gitea ssh port:
  export POD_NAME=$(kubectl get pods --namespace ttc-infra -l "app=git-gitea" -o jsonpath="{.items[0].metadata.name}")
